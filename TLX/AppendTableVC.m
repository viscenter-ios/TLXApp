...
///////////////////////////////////////////////////////////////////////////////////////////
    //This function grabs the information of the experiment found at the selected row, retrieves the associated questionfiles information and then loads the DynamicFormVC for appending.
- (void)tableView:(UITableView *)tbleView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
  ...
  if(errorFlag){
    ...
    //This is similar to the saveData functions used in DynamicFormVC and QFileGrabberVC. Rather than saving, the question file is retrieved and then information pulled from it.
    NSError *error;
	...
	if([fetchedObjects count] < 1){
        ...
        [form setQuestionFileName:[e valueForKey:@"qFileName"]];
        NSLog(@"qFileName: %@", [e valueForKey:@"qFileName"]);
        NSLog(@"fileName: %@", [e valueForKey:@"fileName"]);
        ...
        [form setFileName:[e valueForKey:@"fileName"]];
        [self.navigationController pushViewController:form animated:YES];
        [form setSubjectName:[subjectText text]];
        int n = [[e valueForKey:@"lastTrial"] intValue];
        NSLog(@"TrialTextValue: %d", n);
        NSLog(@"NEW TrialTextValue: %d", n+1);
        [form setTrialTextValue:[NSString stringWithFormat:@"%d", n+1]];
        [form setTrialNum:n+1];
    }
