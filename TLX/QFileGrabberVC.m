...
-(void)parseQFile {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Save File" style: UIBarButtonItemStyleBordered target:self action:@selector(saveQFile)];
}
//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\
...
///////////////////////////////////////////////////////////////////////////////////////////
-(void) saveFirst{
    qFileString = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"NASAQ" ofType:@"txt"] usedEncoding:NO error:Nil]; 
    scanner = [NSScanner scannerWithString:qFileString];
    ...
    [scanner scanUpToString:@"\n" intoString:&temp];
    [self setRangeIncrement:[NSString stringWithFormat:@"%@", temp]];
    NSLog(@"setRangeIncrement: %@", temp);
    [self setQuestionNum:[NSNumber numberWithInt:[numOfQuestions intValue]]];
    [self setRangeInc:[NSNumber numberWithInt:[rangeIncrement intValue]]];
    NSLog(@"setRangeInc: %d", [rangeIncrement intValue]);
...
///////////////////////////////////////////////////////////////////////////////////////////
    //This function downloads the file from the URL entered in the addressText box. It then displays the contents of the file in the textview and begins parsing the file.
-(IBAction)downloadQFile{
    NSURL *url = [NSURL URLWithString:[addressText text]];
    [self setQFileString:[NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:nil]];
    if(url){
...
-(IBAction) createQFile{
    //QFileCreator *settings = [[QFileCreator alloc] init];
    [self.navigationController pushViewController:[[QFileCreator alloc] init] animated:YES];
}
//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\
